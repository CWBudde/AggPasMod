...
// ----------------------------------------------------------------------------
// -Grain Geometry - Version 2.4 (Public License)
// Copyright (C) 2002-2005 Maxim Shemanarev (http://www.antigrain.com)
//
// -Grain Geometry - Version 2.4 Release Milano 3 (AggPas 2.4 RM3)
...
type
  ...
  TAggFontEngineWin32TrueTypeBase = class(TAggFontEngine)
  private
    FFlag32: Boolean;
    FDc: HDC;
  ...
  public
    ...
    // Set Gamma
    procedure SetGamma(F: TAggVertexSource);
    ...
    function GetCharSet: DWORD;
    function GetPitchAndFamily: DWORD;
...
function Fx_to_dbl(P: PFixed): Double;
begin
  Result := P.Value + P.Fract * (1.0 / 65536.0);
end;

function Fx_to_plainInt(Fx: PFixed): Integer;
begin
  Result := Integer(Fx);
end;

function Fx_toInt26p6(P: PFixed): Integer;
begin
  Result := (Integer(P.Value) shl 6) + (ShrInt32(Integer(P.Fract), 10));
end;

function Dbl_toInt26p6(P: Double): Integer;
begin
...
var
  Cur_glyph, End_glyph, End_poly, Cur_poly: PInt8u;
...
begin
  Cur_glyph := Gbuf;
  End_glyph := PInt8u(PtrComp(Gbuf) + TotalSize * SizeOf(Int8u));

  while PtrComp(Cur_glyph) < PtrComp(End_glyph) do
  begin
    Th := PTTPolygonHeader(Cur_glyph);

    End_poly := PInt8u(PtrComp(Cur_glyph) + Th.Cb);
    Cur_poly := PInt8u(PtrComp(Cur_glyph) + SizeOf(TTPOLYGONHEADER));
    ...
    Path.MoveTo(Dbl_toInt26p6(X), Dbl_toInt26p6(Y));

    while PtrComp(Cur_poly) < PtrComp(End_poly) do
    begin
      Pc := PTTPolyCurve(Cur_poly);
      ...
      begin
        ...
        while I < Pc.Cpfx do
        begin
          X := Fx_to_dbl(@PPointfx(PtrComp(@Pc.Apfx) + I *
            SizeOf(POINTFX)).X);
          Y := Fx_to_dbl(@PPointfx(PtrComp(@Pc.Apfx) + I *
            SizeOf(POINTFX)).Y);
          ...
          Mtx.Transform(Mtx, @X, @Y);
          Path.LineTo(Dbl_toInt26p6(X), Dbl_toInt26p6(Y));
      ...
      begin
        ...
        begin
          ...
          Path.Curve3To(Dbl_toInt26p6(X), Dbl_toInt26p6(Y), Dbl_toInt26p6(X2),
            Dbl_toInt26p6(Y2));
...
procedure TAggFontEngineWin32TrueTypeBase.SetGamma;
begin
  FRasterizer.Gamma(F);
end;
...
function TAggFontEngineWin32TrueTypeBase.GetResolution;
begin
  Result := FResolution;
end;
...
function TAggFontEngineWin32TrueTypeBase.GetWeight;
begin
  Result := FWeight;
end;

function TAggFontEngineWin32TrueTypeBase.GetItalic;
begin
  Result := FItalic;
end;

function TAggFontEngineWin32TrueTypeBase.GetCharSet;
begin
  Result := FCharSet;
end;

function TAggFontEngineWin32TrueTypeBase.GetPitchAndFamily;
begin
  Result := FPitchAndFamily;
end;
...
function TAggFontEngineWin32TrueTypeBase.AddKerning;
var
  End_, Beg_, Mid: Integer;
...
begin
  ...
  begin
    ...
    begin
      Mid := (End_ + Beg_) div 2;
      Kp := PKerningPair(PtrComp(FKerningPairs) + Mid *
        SizeOf(TKerningPair));
      ...
      begin
        ...
        if (FGlyphRendering = grOutline) or
          ...
          FAffine.Transform_2x2(FAffine, @Dx, @Dy);
