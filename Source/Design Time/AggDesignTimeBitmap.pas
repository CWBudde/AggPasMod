...
{$ENDIF}
  ...
  SysUtils, Classes, Clipbrd, Agg2D;
...
{$ENDIF}
    AlphaChannel: TAggImage32;
    RGBChannels: TAggImage32;
    procedure AlphaChannelMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer; Layer: TAggCustomLayer);
    procedure RGBChannelsMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer; Layer: TAggCustomLayer);
    function CurrentImage: TAggImage32;
  public
...
{$ENDIF}
  ...
  TAggImage32Editor = class(TComponentEditor)
  public
...
function TPictureEditorForm.CurrentImage: TAggImage32;
begin
...
procedure TAggImage32Editor.ExecuteVerb(Index: Integer);
var
  Img: TAggCustomImage32;
  BitmapEditor: TBitmap32Editor;
begin
  Img := Component as TAggCustomImage32;
  if Index = 0 then
  begin
    BitmapEditor := TBitmap32Editor.Create(nil);
    try
...
function TAggImage32Editor.GetVerb(Index: Integer): string;
begin
  if Index = 0 then Result := 'Bitmap32 Editor...';
end;

function TAggImage32Editor.GetVerbCount: Integer;
begin
  Result := 1;
end;

procedure TPictureEditorForm.AlphaChannelMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer; Layer: TAggCustomLayer);
var
  P: TPoint;
begin
